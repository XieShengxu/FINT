...
class PacketIn:
    def __init__(self, log_level="", iface='eth0', method='msg', bitmap_task=0b1010001000100001):
    ...
    def handle_pkt(self, pkt):
        ...
        if eth.type == 0x1727:
            ...
            for i in range(int_info.count):
                ...
                self.log.debug(bitmap_add_task)
                if bitmap_add_task == self.bitmap_task:
                    self.log.info("bitmap added done: %s" % bin(bitmap_add_task)[2:])
                ...
                for j in range(self.bitmap_task_len):
                    ...
                    if (bitmap_add_task & position) == position:
                        ...
                        value = int(str(binascii.b2a_hex(data[width: width+metadata_width[self.method][position]])), 16)
...
def get_args():
    help = "bitmap_task msg: 0b1010001000100001 => 41505\n" \
    ...
    parser.add_argument('-l', '--log_level', help='Log level, info or debug.',
                        type=str, required=False, default='info')
    parser.add_argument('-i', '--iface', help='Name of interface.',
                        type=str, required=False, default='ctl-eth17')
    parser.add_argument('-m', '--method', help='Method of msg or msr.',
                        type=str, required=False, default="msg")
    parser.add_argument('-bt', '--bitmap-task', help='Bitmap task value.',
...
if __name__ == '__main__':
    ...
    args = get_args()
    packet_in = PacketIn(iface=args.iface, method=args.method, bitmap_task=args.bitmap_task)
    # packet_in = PacketIn()
