...
from p4mininet_utils.p4_mininet import configureP4Switch, P4Host
from p4mininet_utils.p4topo import AppTopo
...
class MininetRunner:
    ...
    def __init__(self, topo_file, log_dir, pcap_dir,
                 switch_json, bmv2_exe='simple_switch', quiet=False, log_console=False, controlplane=False):
    ...
    def parse_links(self, unparsed_links):
        ...
        default_delay = None  # '0ms'
        default_bw = None  # Mbps
        default_loss = None
    ...
    def create_network(self):
        ...
        self.topo = AppTopo(hosts=self.hosts, switches=self.switches, links = self.links, log_dir=self.log_dir,
                            bmv2_exe=self.bmv2_exe, controlplane=self.controlplane)
...
def get_args():
    ...
    parser.add_argument('-p', '--pcap-dir', help='Dump packets on interfaces to pcap files',
                        type=str, action="store", required=False, default=default_pcaps)
    parser.add_argument('-j', '--switch_json', help='Path to JSON config file',
                        type=str, action="store", default="")
    parser.add_argument('-b', '--behavioral-exe', help='Path to behavioral executable',
                        type=str, action="store",
                        default="/home/joxie/p4/behavioral-model/targets/simple_switch/simple_switch")
    parser.add_argument('-lc', '--log-console', help='set log console to True.',
                        action='store_true', required=False, default=False)
    parser.add_argument('--thrift-port', help='Thrift server port for table updates',
                        type=int, action="store", default=9090)
    parser.add_argument('-c', '--controlplane', help='set control plane switch and add link to controller',
                        action='store_true', required=False, default=False)
...
if __name__ == '__main__':
    ...
    runner = MininetRunner(args.topo, args.log_dir, args.pcap_dir,
                           args.switch_json, args.behavioral_exe, args.quiet, args.log_console, args.controlplane)
