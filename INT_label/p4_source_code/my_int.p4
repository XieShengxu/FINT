...
control MyIngress(inout headers hdr,
    ...
    apply {
        if(hdr.sr[0].type==1) {
            ...
            hdr.sr_hdr[0].sr_length = hdr.sr_hdr[0].sr_length - 1;
        }
        do_sr();
    }
}
...
control MyEgress(inout headers hdr,
    ...
    action do_int(bit<8> sw_id) {
        hdr.inthdr.push_front(1);
        hdr.inthdr[0].setValid();

        hdr.inthdr[0].sw_id = sw_id;
        hdr.inthdr[0].ingress_port=(bit<8>)standard_metadata.ingress_port;
        hdr.inthdr[0].egress_port=(bit<8>)standard_metadata.egress_port;
        hdr.inthdr[0].ingress_global_timestamp=(bit<48>)standard_metadata.ingress_global_timestamp;
        hdr.inthdr[0].egress_global_timestamp=(bit<48>)standard_metadata.egress_global_timestamp;
        hdr.inthdr[0].enq_timestamp=(bit<32>)standard_metadata.enq_timestamp;
        hdr.inthdr[0].enq_qdepth=(bit<16>)standard_metadata.enq_qdepth;
        hdr.inthdr[0].deq_qdepth=(bit<16>)standard_metadata.deq_qdepth;
