...
parser.add_argument('--behavioral-exe', help='Path to behavioral executable',
                    type=str, action="store", default="/home/joxie/p4/behavioral-model/targets/simple_switch/simple_switch")
...
parser.add_argument('--nodes-list', help='Number of spine, leaf, tor, host, pod',
                    nargs='*', default=[2, 2, 2, 2, 4])
parser.add_argument('--pcap-dump', help='Dump packets on interfaces to pcap files',
                    type=str, action="store", required=False, default=False)
args = parser.parse_args()

TIME_OUT=10*10 #2*20ms
...
def database_init(r, r2, r4, nodes_list=None):
    if nodes_list is None:
        nodes_list = [2, 2, 2, 2, 4]
    r2.flushall()
...
def main():
    ...
    #             h.cmd("python ../packet/receive/receive.py >/dev/null &")  # 定向到"nul"(Windows)或"/dev/null"(Linux)会抑制输出，既不屏显也不存盘
