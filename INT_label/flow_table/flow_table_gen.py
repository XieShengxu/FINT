    ...
    def flow_table_gen(self, nodes_list):
            ...
            for j in xrange(spine_num):
                with open("./flow_table/s%d_s%d.txt" % (i+1, j+1), "w") as f:
                    ...
                    f.write("table_add int_table_sampling do_int => %d\n" %
                            (i*spine_num+j))
                    f.write("table_add int_table_sampling2 do_int => %d\n" %
                            (i*spine_num+j))
                    for k in range(20):
                ...
                with open("./command.sh", "a") as f:
                    f.write("sudo simple_switch_CLI --thrift-port %d <../flow_table/flow_table/s%d_s%d.txt\n"%(thrift_port,i+1,j+1))
                    thrift_port+=1
        # leaf
        for i in xrange(pod_num):
            for j in xrange(leaf_num):
                ...
                with open("./command.sh", "a") as f:
                    f.write("sudo simple_switch_CLI --thrift-port %d <../flow_table/flow_table/p%d_l%d.txt\n"%(thrift_port,i+1,j+1))
                    thrift_port+=1
        ...
        for i in xrange(pod_num):
            for j in xrange(tor_num):
                ...
                with open("./command.sh", "a") as f:
                    f.write("sudo simple_switch_CLI --thrift-port %d <../flow_table/flow_table/p%d_t%d.txt\n"%(thrift_port,i+1,j+1))
                    thrift_port+=1


if __name__ == "__main__":
    flow_table1 = flow_table()
    flow_table1.flow_table_gen([2,2,2,2,4])
