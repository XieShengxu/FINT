...
# Return a Publish/Subscribe object. With this object, you can subscribe
# to channels and listen for messages that get published to them.
pubsub=r.pubsub()
...
while(True):
    ...
    if(message!=None):
        ...
        os.system('sudo simple_switch_CLI --thrift-port %d < ./flow_table_ctrl/switch%s.txt' % (int(sw_id)+9090,sw_id))
        r3.set(data,-1)     # add Key-value
        r3.pexpire(data,10*TIME_OUT)    # Set an expire flag on key ``name`` for ``time`` milliseconds
